...
if(isset($_POST['confirmed'])){
  ob_start();
  header('Location:Badminton_Report.php');
}?>
<!DOCTYPE html>
...
$sql="SELECT Serial_no,Name,Tel_number,Arena,Address,Sports,RacquetQuantity,RacquetBrand,RacquetModel,
StringType,StringTension,StringColor,RacquetCover,
Amount,CollectionDate,PickUpTime,CollectionStatus,DeliveryStatus,PaymentMode FROM Badminton_racquets WHERE PaymentMode=''";

$result = $conn->query($sql);
if ($result->num_rows > 0) {
  ?> <form class="" action="Badminton_Report.php" method="POST">
  ...
  while($row = $result->fetch_assoc()) {
    ?><tr><td style="border: 1px solid black"><input type="checkbox" value="<?php echo $row['Serial_no']?>
      <?php echo "*"?><?php echo $row['Name']?>
      <?php echo "*"?><?php echo $row['Arena'] ?>
      <?php echo "*"?><?php echo $row['Address'] ?>
      <?php echo "*"?><?php echo $row['Tel_number'] ?>
  ...
  <br />
    ...
    <input type="Submit" class="btn btn-success "name="direction" value="Get Directions" />
    <input type="Submit" class="btn btn-primary "name="delivered" value="Delivered" />
    <button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">
      Payment Mode
    </button>
    <div class="dropdown-menu rep">
      <input type="Submit" class="btn btn-dark pmode"name="Cash" value="Cash" />
      <input type="Submit" class="btn btn-primary pmode"name="Gpay" value="Gpay"  />
      <input type="Submit" class="btn btn-primary pmode"name="PhonePe" value="PhonePe"  />
      <input type="Submit" class="btn btn-primary pmode"name="Paytm" value="Paytm" />
...
<?php
$conn=mysqli_connect('localhost','wrSports','Champion1@','stringing-app');
// $due=$conn->query("SELECT SUM(Amount) FROM Badminton_racquets WHERE PaymentMode=''");
// echo $due;
$rmess=$_POST['ur'];
$mess=explode("*",$rmess);
echo $mess[0];
...
// while($row = mysqli_fetch_array($due)){
// echo $row[16];
// }
// $sum = $row['Amount'];
// echo $sum;
// $result2 = $conn->query($due);
// echo $due;
// echo $result2;
$address;
if(isset($_POST['Cash'])){
...
echo $mess[0];
echo $mess[12];
$sql="UPDATE badminton_racquets SET PaymentMode='Cash' WHERE Serial_no='$mess[0]'";
$result = $conn->query($sql);
...
$mess=explode("*",$rmess);
echo $mess[0];
$sql="UPDATE badminton_racquets SET PaymentMode='Gpay' WHERE Serial_no='$mess[0]'";
$result = $conn->query($sql);
if(!mysqli_query($conn,$sql)){
  echo " not inserted";
}else{
...
$mess=explode("*",$rmess);
echo $mess[0];
$sql="UPDATE badminton_racquets SET PaymentMode='PhonePe' WHERE Serial_no='$mess[0]'";
$result = $conn->query($sql);
if(!mysqli_query($conn,$sql)){
  echo " not inserted";
}else{
...
$mess=explode("*",$rmess);
echo $mess[0];
$sql="UPDATE badminton_racquets SET PaymentMode='Paytm' WHERE Serial_no='$mess[0]'";
$result = $conn->query($sql);
if(!mysqli_query($conn,$sql)){
  echo " not inserted";
}else{
...
if(isset($_POST['collected']))
{
  ...
  //echo $rmess;
  $mess=explode("*",$rmess);
    $sql="UPDATE badminton_racquets SET CollectionStatus='Yes' WHERE Serial_no='$mess[0]'";
  //print_r($mess);
 ...
 if($model_count>1 && $brand_count>1 && $stringtype>1){
    $ready="Dear ".$mess[1]. "\nYour ".$brand_count." Badminton Racquets for Stringing with following Details: \n";
   ...
   $ready="Dear ".$mess[1]."\nYour Badminton Racquet ".$mess[6]." ".$mess[7]." has been collected From ".$mess[2]." on ".date("d-m-Y", strtotime($mess[12]))."\n It will be Strung With ".$mess[8]."/".$mess[9]."LBS/".$mess[10]."\n and Total Charges Payable are ".$mess[15]."/- only.\nThanks for choosing STRING-Up \n TX_".$mess[0].".";
 }
...

{
   ...
   $mess=explode("*",$dmess);
    ...
    echo $udate;
    echo $mess[0];
     $sql="UPDATE Badminton_racquets SET DeliveryStatus='Yes' WHERE Serial_no='$mess[0]'";
     $result = $conn->query($sql);
     if(!mysqli_query($conn,$sql)){
       echo " not inserted";
     }else{
...
if($model_count>1 && $brand_count>1 && $stringtype>1){
  ...
  $deliver="Dear ".$mess[1]."\nYour ".$mess[6]." ".$mess[7]." Badminton Racquet which was Strung with ".$mess[8]." @ ".$mess[9]." Lbs Tension are Delivered to you on ".date("d-m-Y")."\nThanks for choosing STRING-Up\nTx-".$mess[0].".";
}
   echo $deliver;
  $url="http://alerts.prioritysms.com/api/web2sms.php";
...
for (let i=1;i<table.rows.length;i++){
  sumval=sumval+table.rows[i].cells[13].innerHTML;
}
console.log(sumval);
