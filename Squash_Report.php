...
Amount,CollectionDate,PickUpTime,CollectionStatus,DeliveryStatus,PaymentMode FROM squash_racquets";
...
if(isset($_POST['collected']))
{
  ...
  //echo $rmess;
  $mess=explode("*",$rmess);
  $sql="UPDATE squash_racquets SET CollectionStatus='Yes' WHERE Serial_no='$mess[0]'";
  //print_r($mess);
 ...
 if($model_count>1 && $brand_count>1 && $stringtype>1){
    $ready="Dear ".$mess[1]. "\nYour ".$brand_count." Squash Racquets for Stringing with following Details: \n";
   ...
   $ready="Dear ".$mess[1]."\nYour Squash Racquet ".$mess[6]." ".$mess[7]." has been collected From ".$mess[2]." on ".date("d-m-Y", strtotime($mess[12]))."\n It will be Strung With ".$mess[8]."/".$mess[9]."LBS/".$mess[10]."\n and Total Charges Payable are ".$mess[15]."/- only.\nThanks for choosing STRING-Up \n TX_".$mess[0].".";
 }
...

{
   ...
   $mess=explode("*",$dmess);
    ...
    echo $udate;
    echo $mess[0];
     $sql="UPDATE squash_racquets SET DeliveryStatus='Yes' WHERE Serial_no='$mess[0]'";
     $result = $conn->query($sql);
...
if($model_count>1 && $brand_count>1 && $stringtype>1){
  ...
  $deliver="Dear ".$mess[1]."\nYour ".$mess[6]." ".$mess[7]." Squash Racquet which was Strung with ".$mess[8]." @ ".$mess[9]." Lbs Tension are Delivered to you on ".date("d-m-Y")."\nThanks for choosing STRING-Up\nTx-".$mess[0].".";
}
   echo $deliver;
  $url="http://alerts.prioritysms.com/api/web2sms.php";
